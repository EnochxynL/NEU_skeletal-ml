...
def Print3D(num_frame, point, arms, rightHand, leftHand, legs, body):
    ...
    for i in range(n, m):
        ...
        # 画出两个body所有关节
        plot3D.scatter(point[0, i, :, :]*Expan_Multiple, point[1, i, :, :]*Expan_Multiple, point[2, i, :, :], c='red', s=40.0) # c: 颜色;  s: 大小
...
def main():
    if len(sys.argv) < 2:
        print('Usage: uv run skeletal-show <path/to/file.skeleton>')
        sys.exit(1)
